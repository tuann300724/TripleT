    ...
    window.tripletVnpayCreatePayment = async function (opts) {
        ...
        for (let key of sortedKeys) {
            ...
            if (val !== null && val !== undefined && val !== "") {
                signArr.push(vnpEncode(key) + "=" + vnpEncode(val));
                queryArr.push(vnpEncode(key) + "=" + vnpEncode(val));
            }
        }

        let signData = signArr.join("&");
        let queryData = queryArr.join("&");
